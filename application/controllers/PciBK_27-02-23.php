...
class pci extends CI_Controller
{
    ...
    public function search_noscan($no_scan,$thn)
    {
        $dataSearch = $this->db->query("SELECT *,
                                                ...
                                                a.tgl_tetap AS periode_awal,
                                                ( a.tgl_tetap + INTERVAL '12' MONTH - INTERVAL '1' DAY) AS periode_akhir,
    ...
    public function generate_proses()
    {
        ...
        if ($periode == "gaji_atas") {
            $sheet = $this->db->query("SELECT *, MONTHNAME( tgl_tetap ) AS awal, MONTHNAME( ( tgl_tetap + INTERVAL '12' MONTH ) ) AS akhir FROM tbl_makar WHERE status_karyawan = 'Tetap' AND gaji = 'atas' AND status_aktif = 1 ORDER BY ( tgl_tetap + INTERVAL '12' MONTH ) DESC")->result_array(); 
            ...
            $sheet = $this->db->query("SELECT *,
                                                DATE_FORMAT( tgl_tetap, '%d %M %Y' ) AS awal,
                                                DATE_FORMAT( ( tgl_tetap + INTERVAL '12' MONTH - INTERVAL '1' DAY), '%d %M %Y' ) AS akhir 
                                        FROM
                                            tbl_makar 
                                        WHERE
                                            ...
                                            AND gaji IS NULL 
                                            AND status_aktif = 1 
                                            AND DATE_FORMAT( tgl_tetap, '%d %M %Y' ) = '$periode' 
                                        ORDER BY
