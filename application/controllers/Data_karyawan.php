...
class Data_Karyawan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $dataEmployees = $this->db->query("SELECT no_scan,
                                        nama,
                                        dept,
                                        jabatan,
    ...
    public function print_datakaryawan($no_scan)
    {
        ...
        // Query untuk mengambil data karyawan berdasarkan no_scan yang sudah didekripsi
        $query = $this->db->query("
        SELECT *, 
            DATE_FORMAT(tk.kontrak_awal, '%d') AS tgl_masuk_hari, 
            DATE_FORMAT(tk.kontrak_awal, '%M') AS tgl_masuk_bulan, 
            DATE_FORMAT(tk.kontrak_awal, '%Y') AS tgl_masuk_tahun,
            SUBSTR(makar.alamat_domisili, 1, LOCATE('RT', makar.alamat_domisili)-1) as Domisili,
            SUBSTR(SUBSTR(makar.alamat_domisili, LOCATE('/', makar.alamat_domisili)+ 1), LOCATE('/', SUBSTR(makar.alamat_domisili, LOCATE('/', makar.alamat_domisili)+ 1))+ 13) AS kelurahan
        FROM tbl_makar makar  
        LEFT JOIN tbl_kontrak tk ON tk.no_scan = makar.no_scan 
        WHERE makar.no_scan = '$no_scan_decrypt'
    ");

        // Periksa apakah query mengembalikan hasil
        if ($query->num_rows() > 0) {
            // Ambil data karyawan dari hasil query
            $data['makar'] = $query->row();
            ...
            $this->load->view('403/403');
            // echo "Data karyawan dengan no_scan $no_scan_decrypt tidak ditemukan.";
        }
    }
    ...
    //     WHERE makar.no_scan = '$no_scan_decrypt'")->row();
    //     $data['no_scan'] = $no_scan_decrypt;
    ...
    public function tanda_tangan($no_scan)
    {
        ...
        $query = $this->db->get_where('tbl_makar', array('no_scan' => $no_scan_decrypt))->row();
        ...
        if ($query->num_rows() > 0) {
            ...
            // Memuat view tanda tangan ('ttd') dan mengirimkan data ke view tersebut
            $this->load->view('data_karyawan/ttd', $data);
        } else {
            // Handle jika data tidak ditemukan
            $this->load->view('403/403');
    ...
    public function edit_employee($no_scan)
    {
        ...
        // Query untuk mengambil data karyawan dan kontrak berdasarkan no_scan yang sudah didekripsi
        $data['user'] = $this->db->get_where('user', array('name' => $this->session->userdata('name')))->row_array();
        $data['makar'] = $this->db->get_where('tbl_makar', array('no_scan' => $no_scan_decrypt))->row();
        $tbl_makar = $this->db->get_where('tbl_makar', array('no_scan' => $no_scan_decrypt))->row();
        $query = $this->db->query("SELECT count(*) as id, no_scan, kontrak_awal, kontrak_akhir FROM `tbl_kontrak` WHERE no_scan = '$no_scan_decrypt' AND keterangan = '$tbl_makar->status_karyawan' ORDER BY id DESC LIMIT 1")->row();


        // Periksa apakah query mengembalikan hasil
        if ($query->num_rows() > 0) {
            // Ambil data karyawan dari hasil query
            $data['kontrak'] = $query->row();
            $data['no_scan'] = $no_scan_decrypt;
