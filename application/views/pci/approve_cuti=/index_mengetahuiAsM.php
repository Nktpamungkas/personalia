					...
					<script type="text/javascript">
						$(document).ready(function () {
							...
							function tampil_data_employee() {
								$.ajax({
									type: 'ajax',
									url: '<?= base_url() ?>pci/data_approve_cuti_mengetahuiASM',
									async: false,
									dataType: 'json',
										...
										for (var i = 0; i < data.length; i++) {
											html += '<tr class="gradeX">' +
												...
												<div class="modal fade" id="modalResign` + data[i].id + `" tabindex="-1" role="dialog" aria-labelledby="modalResign" aria-hidden="true">
													<form role="form" action="<?= base_url('pci/approval_cuti1ASM/') . $user['name']; ?>" method="POST">
												...
												data[i].nama + '/' + data[i].nip + `/<?= date('Y-m-d h:i:s'); ?>" name="getalldata" type="hidden">
																			...
																			<input class="form-control input-sm" value="` + data[i].jabatan + `" name="jabatan_1" type="text">
