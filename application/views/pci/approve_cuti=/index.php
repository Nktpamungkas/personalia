					...
					<script type="text/javascript">
						$(document).ready(function () {
							...
							function tampil_data_employee() {
								$.ajax({
									type: 'ajax',
									url: '<?= base_url() ?>pci/data_approve_cuti',
									async: false,
									dataType: 'json',
											...
											html +=
												...
												data[i].id + `"><i class="fa fa-sign-out"></i>Approve Pengajuan Cuti</button>
													...
													<div class="modal fade" id="modalResign` + data[i].id + `" tabindex="-1" role="dialog" aria-labelledby="modalResign" aria-hidden="true">
														<form role="form" action="<?= base_url('pci/approve_cuti/') . $user['name']; ?>" method="POST">
													...
													.fkode_cuti + '/' + data[i].nama + '/' + data[i].nip + `/<?= date('Y-m-d h:i:s'); ?>" name="getalldata" type="hidden">
																				<input class="form-control input-sm" value="ANGGORO BAYU MUKTI" name="user_approval" type="hidden">
																				<input class="form-control input-sm" value="MANAGER CPL" name="jabatan" type="hidden">
