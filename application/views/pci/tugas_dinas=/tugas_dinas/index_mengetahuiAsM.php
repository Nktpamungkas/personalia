<section id="main-content">
	<t class="wrapper">
		<h4><i class="fa fa-angle-right"></i> Time Attendance <i class="fa fa-angle-right"></i> Form Pengajuan Dinas
		</h4>
		<div class="row mb col-sm-12">
			...
			<div class="content-panel">
				<div class="adv-table">
					...
					<script type="text/javascript" src="<?= base_url(); ?>lib/advanced-datatable/js/jquery.js"></script>
					<script type="text/javascript"
						src="<?= base_url(); ?>lib/advanced-datatable/js/jquery.dataTables.js"></script>
					<script type="text/javascript">
						$(document).ready(function () {
							tampil_data_employee(); // Memanggil fungsi untuk tampilkan data.
							...
							function tampil_data_employee() {
								$.ajax({
									type: 'ajax',
									url: '<?= base_url() ?>pci/data_tugas_dinas_mengetahui_',
									async: false,
									dataType: 'json',
										...
										for (var i = 0; i < data.length; i++) {
											html += '<tr class="gradeX">' +
												...
												<div class="modal fade" id="modalResign` + data[i].id + `" tabindex="-1" role="dialog" aria-labelledby="modalResign" aria-hidden="true">
													<form role="form" action="<?= base_url('pci/approval_tugas_dinas1_/') . $user['name']; ?>" method="POST">
														<div class="modal-dialog">
															<div class="modal-content">
																<div class="modal-header">
																	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
																	<h4 class="modal-title" id="myModalLabel">Approval Dinas</h4>
																</div>
																<div class="modal-body">
																	<div class="form-group">
																		...
																		<div class="col-lg-5">
																			<label class="control-label col-lg-12">Alasan Cuti/Izin</label>
																		</div>
																		<div class="col-lg-7">
																		...
																		<div class="col-lg-7">
																			<input class="form-control input-sm" value="` + data[i].tgl_selesai + `" name="tgl_selesai" readonly>
																		</div>
																		<div class="col-lg-5">
																			<label class="control-label col-lg-12">Approval</label>
																		</div>
																		<div class="col-lg-7">
																			<input type="radio" name="ket_approval_1" value="Approved" id="approveRadio"> Approved <br/>
																			<input type="radio" name="ket_approval_1" value="Rejected" id="rejectRadio"> Rejected
																			<input class="form-control input-sm" value="<?= $user['name']; ?>/` + data[i].fkode_cuti + '/' +
												data[i].nama + '/' + data[i].nip + `/<?= date('Y-m-d h:i:s'); ?>" name="getalldata" type="hidden">
																			<input class="form-control input-sm" value="` + data[i].atasan1 + `" name="user_approval_1" type="text">
																			<input class="form-control input-sm" value="` + data[i].jabatan + `" name="jabatan_1" type="hidden">
																...
																<div class="modal-footer">
																	...
																	<button type="submit" class="btn btn-primary" id="approveButton" disabled>Approve</button>
																</div>
															</div>
														</form>
													</div>
...
<script>
	$(document).ready(function () {
		...
		// Enable/Disable Approve button based on radio selection
		$('input[name="ket_approval_1"]').on('change', function () {
			$('#approveButton').prop('disabled', false); // Enable the button when a radio is selected
		});
	});
</script>