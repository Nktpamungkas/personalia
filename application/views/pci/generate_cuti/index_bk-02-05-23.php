                            ...
                            <?php 
                                ...
                                if ($dept == "HRD") {
                                    $data = $this->db->query("SELECT *, 
                                                                ...
                                                                ORDER BY 
                                                                    dept ASC, no_scan ASC")->result_array(); 
                                } else {
                                    $data = $this->db->query("SELECT *, 
                                                                    DATE_FORMAT( tgl_tetap, '%d %M' ) AS awal,
                                                                    case 
                                                                        when year( tgl_tetap) =  YEAR(CURDATE())
                                                                        then year( tgl_tetap + interval '1' YEAR)
                                                                        else YEAR(CURDATE())
                                                                    end as thn_awal,
                                                                    DATE_FORMAT( ( tgl_tetap + INTERVAL '12' MONTH - INTERVAL '1' DAY), '%d %M' ) AS akhir, 
                                                                    case 
                                                                        when year( tgl_tetap) =  YEAR(CURDATE())
                                                                        then year( tgl_tetap + interval '2' YEAR)
                                                                        else  YEAR(CURDATE()+interval '1'year)
                                                                    end as thn_akhir,
                                                                    gaji  
                                                                FROM 
                                                                    tbl_makar 
