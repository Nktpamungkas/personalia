...
function _sk() {
	...
	if (statuskaryawan === "Kontrak1" || statuskaryawan === "Kontrak2" || statuskaryawan === "Kontrak" ||
		statuskaryawan === "Resigned") {
...
$(document).ready(function() {
	...
	document.getElementById('tgl_lahir').addEventListener('change', function() {
		...
		var monthDiff = today.getMonth() - tgl_lahir.getMonth();
		if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < tgl_lahir.getDate())) {
			age--;
		}
		...
		if (age < 17) {
			ageMessage.textContent = 'Umur dibawah 17 tahun';
		...
		<div class="row mt">
			<div class="col-lg-12">
				<h4><i class="fa fa-angle-right"></i><a href="<?= base_url('data_karyawan'); ?>"> Employee Information
					</a><i class="fa fa-angle-right"></i> Add Form Employee</h4>
				<?= $this->session->flashdata('message'); ?>
				<form class="form-style-4" action="<?= base_url('data_karyawan/add/') . $user['name']; ?>"
					method="post">
						...
						<div class="form">
									...
									<tr>
										...
										<th>
											<label>
												...
												<input type="number" value="<?= set_value('no_scan'); ?>" name="no_scan"
										...
										<th>
											<label>
												...
												<input type="text" value="<?= set_value('nama'); ?>"
									...
									<tr>
										<th>
											<label>
												...
												<input type="text" value="<?= set_value('tempat_lahir'); ?>"
										...
										<th>
											<label>
												...
												<input type="date" value="<?= set_value('tgl_lahir'); ?>"
										...
										<th>
											<label>
												...
												<input type="text" onkeyup="this.value = this.value.toUpperCase();"
													value="<?= set_value('alamat_domisili'); ?>"
													placeholder="Alamat Sesuai KTP" name="alamat_ktp" required>
											</label>
										</th>
									</tr>
									<tr>
										<th>
											<label>
												...
												<input type="text" onkeyup="this.value = this.value.toUpperCase();"
													value="<?= set_value('alamat_domisili'); ?>" name="alamat_domisili"
										...
										<th>
											<label>
												...
												<input type="number" name="RT" value="<?= set_value('RT'); ?>"
										...
										<th>
											<label>
												...
												<input type="number" name="RW" value="<?= set_value('RW'); ?>"
										...
										<th>
											<label>
												...
												<input type="text" name="kabupaten_domisili"
													value="<?= set_value('kabupaten_domisili'); ?>" required>
											</label>
										</th>
										<th>
											<label>
												...
												<input type="text" name="kecamatan_domisili"
													value="<?= set_value('kecamatan_domisili'); ?>" required>
											</label>
										</th>
										<th>
											<label>
												<span>Kode Pos <span style="color: red">(*)</span></span><br>
												<input type="number" value="<?= set_value('kode_pos'); ?>"
										...
										<th>
											<label>
												...
												<select name="status_rumah" class="select2"
													...
													<?php foreach ($queryReligion as $dr): ?>
														<option value="<?= $dr['code'] ?>" <?php if ($dr['code'] == set_value('status_rumah')) {
															  echo "SELECTED";
														  } ?>>
															<?= $dr['deskripsi'] ?>
														</option>
							...
							<th>
								<label>
									...
									<input value="<?= set_value('nama_sekolah'); ?>" name="nama_sekolah" type="text"
							...
							<tr>
								<th>
									<label>
										...
										<input value="<?= set_value('jurusan'); ?>" name="jurusan" type="text"
											placeholder="Jurusan">
									</label>
								</th>
								<th>
									<label>
										...
										<input value="<?= set_value('ipk'); ?>" name="ipk" type="number"
							...
							<tr>
								<th>
									<label>
										...
										<input value="<?= set_value('email_pribadi'); ?>" name="email_pribadi"
											type="text" placeholder="Email Pribadi" required>
									</label>
								</th>
								<th>
									<label>
										<span>Pengalaman Kerja </span><br>
										<input value="<?= set_value('pengalaman_kerja'); ?>" name="pengalaman_kerja"
								...
								<th>
									<label>
										...
										<input value="<?= set_value('no_hp'); ?>" name="no_hp" type="text"
							...
							<th>
								<label>
									...
									<input value="<?= set_value('kartu_keluarga'); ?>" name="kartu_keluarga" type="text"
							...
							<th>
								<label>
									...
									<input value="<?= set_value('masa_berlaku_ktp'); ?>" name="masa_berlaku_ktp"
										type="date">
							...
							<th>
								<label>
									...
									<select class="select2" name="status_seragam"
										data-placeholder="Pilih Status Seragam dan Id Card">
										<option value="" disabled selected>
											---------------------------------------------------</option>
										<?php $queryseragam = $this->db->get('status_idcseragam')->result_array(); ?>
										<?php foreach ($queryseragam as $dg): ?>
											<option value="<?= $dg['status_seragam'] ?>" <?php if ($dg['status_seragam'] == set_value('status_seragam')) {
												  echo "SELECTED";
											  } ?>>
												<?= $dg['status_seragam'] ?>
											</option>
								...
								<label>
									<span>Suku<span style="color: red">(*)</span></span>
									<select class="select2" name="suku" data-placeholder="Pilih Suku">
										<option value="" disabled selected>
											---------------------------------------------------</option>
										<?php $querysuku = $this->db->get('suku')->result_array(); ?>
										<?php foreach ($querysuku as $dsk): ?>
											<option value="<?= $dsk['id'] ?>" <?php if ($dsk['id'] == set_value('id')) {
												  echo "SELECTED";
											  } ?>><?= $dsk['nama_suku'] ?></option>
										<?php endforeach; ?>
									</select>
								</label>
							</th>
				...
				<table width="100%">
					...
					<tbody id="divSite">
						<tr>
							...
							<td>
								<label>
									<input type='text' name='pekerjaan[]' class='form-control input-sm'
				...
				<table width="100%">
					...
					<tbody id="divSite_pk">
						<tr>
							...
							<td>
								<label>
									<input type='text' name='dept_perusahaan[]' class='form-control input-sm'
										placeholder="Departemen">
								</label>
							</td>
							<td>
								<label>
									<input type='text' name='jabatan_perusahaan[]' class='form-control input-sm'
										placeholder="Jabatan">
			...
			<div class="form-panel">
				<table width="100%" border="0">
							...
							<label>
								...
								<select class="select2" data-placeholder="Pilih Satu Departemen" name="dept" required>
									...
									<?php foreach ($querydept as $dd): ?>
										<option value="<?= $dd['code']; ?>" <?php if ($dd['code'] == set_value('code')) {
											  echo "SELECTED";
										  } ?>>
							...
							<label>
								...
								<select class="select2" data-placeholder="Pilih Satu Jabatan" name="jabatan" required>
									...
									<?php foreach ($queryJab as $dj): ?>
										<option value="<?= $dj['jabatan']; ?>" <?php if ($dj['jabatan'] == set_value('jabatan')) {
											  echo "SELECTED";
										  } ?>>
											<?= $dj['jabatan']; ?>
										</option>
					...
					<tr>
							...
							<label>
								...
								<select class="select2" data-placeholder="Pilih Satu Golongan" name="golongan" required>
									...
									<?php foreach ($queryGolongan as $dg): ?>
										<option value="<?= $dg['golongan']; ?>" <?php if ($dg['golongan'] == set_value('golongan')) {
											  echo "SELECTED";
										  } ?>>
											<?= $dg['golongan'] ?>
										</option>
							...
							<label>
								...
								<select class="select2" data-placeholder="Pilih Satu Bagian" name="bagian" required>
									...
									<?php foreach ($queryBagian as $db): ?>
										<option value="<?= $db['bagian']; ?>" <?php if ($db['bagian'] == set_value('bagian')) {
											  echo "SELECTED";
										  } ?>>
											<?= $db['bagian']; ?>
										</option>
						...
						<th>
							<label>
								<span>Tgl Masuk Karyawan (bulan/tgl/tahun) <span
										style="color: red">(*)</span></span><br>
								<!-- <input type="date" value="<?= set_value('tgl_masuk'); ?>" name="tgl_masuk" placeholder="Tanggal Masuk Karyawan" required> -->
								<input type="date" id="tgl_masuk" name="tgl_masuk" placeholder="Tanggal Masuk Karyawan"
						...
						<th>
							<label>
								...
								<select name="status_aktif" required>
									...
									<option value="1" <?php if (set_value('status_aktif') == 1) {
										echo "SELECTED";
									} ?>>
										Aktif</option>
									<option value="0" <?php if (set_value('status_aktif') == 0) {
										echo "SELECTED";
									} ?>>
										Tidak Aktif</option>
								</select>
					...
					<tr>
						<th>
							<label>
								...
								<input name="no_bpjs_tk" type="text" value="<?= set_value('no_bpjs_tk'); ?>"
						...
						<th>
							<label>
								...
								<input name="no_bpjs_kes" type="text" value="<?= set_value('no_bpjs_kes'); ?>"
					...
					<tr>
						<th>
							<label>
								...
								<input name="alamat_npwp" type="text" value="<?= set_value('alamat_npwp'); ?>"
							...
							<label>
								...
								<select class="select2" data-placeholder="Kode Jabatan" name="kode_jabatan" width="80%">
									...
									<?php foreach ($queryAdditional as $da): ?>
										<option value="<?= $da['kode']; ?>" <?php if ($da['kode'] == set_value('kode')) {
											  echo "SELECTED";
										  } ?>>
											<?= $da['kode'] . ' - ' . $da['jabatan']; ?>
										</option>
						...
						<th>
							<label>
								...
								<input type="date" id="tgl_evaluasi" name="tgl_evaluasi"
									placeholder="Tanggal Evaluasi Karyawan" readonly>
							</label>
					...
					<tr>
						<th><br><br>
							<label>
								<button id="saveBtn" class="btn btn-primary btn-sm" type="submit"
