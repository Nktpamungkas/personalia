<script language="javascript">
    function tambahNama() {
        ...
        var stre;
        stre = "<div class='col-lg-2' class='form-group has-success'><p id='srow" + idf + "'><select class='form-control input-sm select2' tabindex='2' name='no_scan[]' required><option value='' disabled selected></option><?php $this->db->order_by('nama', 'ASC'); $this->db->where('dept', $user['dept']); $this->db->where('status_aktif', '1'); $queryShift = $this->db->get('tbl_makar')->result_array(); ?> <?php foreach ($queryShift as $dk) : ?> <option value='<?= $dk['no_scan'] ?>' <?php if ($dk['no_scan'] == set_value('no_scan')) { echo 'SELECTED';} ?>><?= $dk['no_scan'].' - '.$dk['nama']; ?></option><?php endforeach ?></select><a href='#' style=\"color:#3399FD;\" onclick='hapusElemen(\"#srow" + idf + "\"); return false;'>Hapus</a></p></div>";
        $("#divSite").append(stre);
        idf = (idf - 1) + 2;
...
<section id="main-content">
    ...
    <h4><i class="fa fa-angle-right"></i><a href="<?= base_url('pkl'); ?>"> Time Attendance </a><i class="fa fa-angle-right"></i> <a href="<?= base_url('pkl2'); ?>"> Form Lembur </a><i class="fa fa-angle-right"></i> Edit Form Lembur</h4>
        ...
        <div class="row mt">
            <div class="col-lg-12">
                <div class="form-panel">
                    <div class="form">
                        <form class="cmxform form-horizontal style-form" action="<?= base_url('pkl2/tambah_proses/') . $user['name']; ?>" method="post">
                            ...
                            <div class="form-group ">
                                ...
                                <div class="col-lg-10">
                                    <select class="form-control input-sm select2" name="status_tipe_lembur" disabled>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-10">
                                    <input class="form-control input-sm" value="<?= $user['name']; ?>" name="dept" type="text" readonly>
                                </div>
                            </div>
                            <div class="form-group">
